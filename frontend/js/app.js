...
/* ==============================================================================
   6. HEADLINE TICKER
   ============================================================================== */
const HOLDOUT_HEADLINES = [
  { headline: "IRGC fast boats intercept commercial tanker in Strait of Hormuz", corridor: "Hormuz", pred: 8.0, trueScore: 8.5, reason: "Fast-boat interdiction represents kinetic threat to Persian Gulf oil corridor." },
  { headline: "Houthi anti-ship ballistic missiles target Greek crude carrier in Gulf of Aden", corridor: "Bab-el-Mandeb", pred: 8.2, trueScore: 8.0, reason: "Direct kinetic missile attack in Red Sea approach chokepoint." },
  { headline: "Indian Oil Corporation announces routine turnaround at Panipat refinery", corridor: "Domestic", pred: 2.0, trueScore: 2.0, reason: "Standard scheduled maintenance with no supply interdiction implications." },
  { headline: "US Treasury designates additional shadow fleet tankers carrying sanctioned oil", corridor: "Suez", pred: 6.0, trueScore: 6.5, reason: "Compliance enforcement increases freight friction on Russian crude voyages." },
  { headline: "Oman reaffirms freedom of navigation and enhances naval patrols near Duqm", corridor: "Hormuz", pred: 3.2, trueScore: 3.5, reason: "Diplomatic assurance and naval security reduces immediate transit hazard." },
  { headline: "GPS spoofing and AIS jamming reported off Iranian coast in Persian Gulf", corridor: "Hormuz", pred: 7.4, trueScore: 7.0, reason: "Electronic warfare tactics elevate tanker collision and interception probability." },
  { headline: "Red Sea tanker insurance war-risk premiums climb 400% following drone wave", corridor: "Bab-el-Mandeb", pred: 7.8, trueScore: 8.0, reason: "Steep insurance surges force VLCC diversions around Cape of Good Hope." }
];
...
function initHeadlineTicker() {
  ...
  const dataset = [...HOLDOUT_HEADLINES, ...HOLDOUT_HEADLINES, ...HOLDOUT_HEADLINES];

  function createPill(item) {
    const pill = document.createElement("div");
    pill.className = "headline-pill";
    const scoreColor = item.pred >= 7 ? "threat-red" : (item.pred >= 4 ? "threat-amber" : "threat-green");

    pill.innerHTML = `
      <span class="pill-tag">${item.corridor}</span>
      <span class="pill-text">${item.headline}</span>
      <span class="pill-score ${scoreColor}">${item.pred.toFixed(1)}</span>
    `;
...
function openHeadlineModal(item) {
  ...
  document.getElementById("modal-pred-score").textContent = item.pred.toFixed(1);
  document.getElementById("modal-true-score").textContent = item.trueScore.toFixed(1);
  
  const delta = (item.pred - item.trueScore).toFixed(1);
  document.getElementById("modal-delta-score").textContent = (delta >= 0 ? `+${delta}` : delta);
  document.getElementById("modal-reason-text").textContent = item.reason;
